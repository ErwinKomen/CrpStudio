...
public class XslTransformer {
	...
	public Integer getParCount(String source) {
		int parCount = StringUtil.countMatches(source, "<p ");
		if (parCount == 0) {
			parCount = StringUtil.countMatches(source, "<event ");
		}
		int headCount = StringUtil.countMatches(source, "<head");
		parCount = parCount + headCount; // plus one for the head paragraph
		return parCount;
	}
	
	public Integer getWordCount(String source) {
		int wordCount = StringUtil.countMatches(source, "<w ");
		return wordCount;
	}
